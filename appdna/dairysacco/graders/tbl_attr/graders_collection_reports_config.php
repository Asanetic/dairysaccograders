...
//"primkey" , "grader_id" , "grader_name" , "phone" , "society_id" , "date_registered" , "hive_site_id" , "hive_site_name" , "password" , "username" , <br><br>
...
/// A.I. NOTES :  please dont omit any keys or variables rewrite new values with code leave keys intact, dont delete commented code , where possible replace it with new commented values if you are capable 
  ...
  $__page_title ="Graders collection report";

  $core_module_configs_=[

    //important
    "primary_table"=>$primary_table__,
    "table_alias" =>"graderscollections",     
    "primary_key"=>"primkey",
    "record_id"=>"grader_id",
  ...
  $novanest_module_ui_blueprint_ = [
    ...
    "db_schema" => [
        ...
        "custom_next_js_query_line_cols" => [
            "collection_history" => [
                "function" => "await mosyFlexQuickSel('milk_collections', `collection_date, quantity_litres, collection_id, session, farmer_name`, ` inner join farmers on milk_collections.farmer_id=farmers.farmer_id where grader_id ='\${row?.grader_id}'`);
",
                "args" => [],
                "return" => "data_res"
            ],
    ...
    "data_behaviour" => [
        ...
        "custom_multi_grid_rows" => [
         ...
         "columns"=>["collection_date","session","quantity_litres","farmer_name","collection_id"],
          ]
