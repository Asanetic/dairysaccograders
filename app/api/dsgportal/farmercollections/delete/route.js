...
import { DeleteFarmerscollectionhistory } from '../farmerscollectionhistory/FarmerscollectionhistoryDbGateway';

export async function GET(request) {

  const { searchParams } = new URL(request.url);

  const deleteToken = searchParams.get('_farmers_delete_record');
  const deleteTokenDecode = base64Decode(deleteToken);

  if (deleteToken) {
    // 👇 Customize table and WHERE clause here
    const table = 'farmers'; // Replace with your actual table
    
     
    
    const whereStr = `WHERE primkey = '${deleteTokenDecode}'`;

    const res = await DeleteFarmerscollectionhistory(deleteTokenDecode, whereStr);
